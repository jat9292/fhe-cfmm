...
abstract contract LBToken is ILBToken {
     ...
     */
    mapping(uint8 => uint32) private _totalSupplies;
     ...
     */
    function totalSupply(uint8 id) public view virtual override returns (uint32) {
     ...
     */
    function balanceOf(address account, uint8 id) public view virtual override returns (uint32) {
    ...
    function balanceOfBatch(address[] calldata accounts, uint8[] calldata ids)
        ...
        returns (uint32[] memory batchBalances)
    {
        batchBalances = new uint32[](accounts.length);
     ...
     */
    function batchTransferFrom(address from, address to, uint8[] calldata ids, uint32[] calldata amounts)
        public
        virtual
        override
     ...
     */
    function _mint(address account, uint8 id, uint32 amount) internal {
     ...
     */
    function _burn(address account, uint8 id, uint32 amount) internal {
        mapping(uint8 => uint32) storage accountBalances = _balances[account];

        uint32 balance = accountBalances[id];
     ...
     * @param amounts The list of amounts to transfer for each token id in `ids`.
     */
    function _batchTransferFrom(address from, address to, uint8[] calldata ids, uint32[] calldata amounts)
        internal
        checkLength(ids.length, amounts.length)
        notAddressZeroOrThis(to)
    {
        mapping(uint8 => uint32) storage fromBalances = _balances[from];
        mapping(uint8 => uint32) storage toBalances = _balances[to];

        for (uint8 i; i < ids.length;) {
            uint8 id = ids[i];
            uint32 amount = amounts[i];

            uint32 fromBalance = fromBalances[id];
